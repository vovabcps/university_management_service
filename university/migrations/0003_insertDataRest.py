...
def makeSystemUserOBJs():
    ...
    totalUserRoles = [[3, rAdmin, True, "@admin.fc.ul.pt"], [110, rProfessor, True, "@professor.fc.ul.pt"],
                      [687, rAluno, False, "@alunos.fc.ul.pt"]]
...
def makePersonalInfoOBJs():
    ...
    for user in allSystemUsers:
        ...
        phone = "9" + str(
            random.randint(00000000, 99999999))  # because they are in portugal so they need a pt phone number xd
        random.seed(user.id)
        ...
        newUSER = PersonalInfo(user=user, address=address, birth_date=date, name=name,
                               phone_number=phone, personal_email=email, gender=gender, nationality=nationality,
                               id_document=idDocument, vat_number=vat)
        newUSER.save()
...
def makeCourseOBJs():
    ...
    for (_, minor) in minors:
        newCourse = Course(name=minor, grau="Minor", credits_number=30, credits_numberByYear="", duration=2,
                           timetable="Diurno", coordinator=allTeachers.pop())
        newCourse.save()
    ...
    # -----------------  licenciaturas  -----------------
    newCourseTI = Course(name="Licenciatura em Tecnologias de Informação", grau="Licenciatura", credits_number=180,
                         credits_numberByYear="1:60|2:60|3:60", duration=6, timetable="Diurno",
                         coordinator=allTeachers.pop())
    newCourseTI.save()

    newCourseEI = Course(name="Licenciatura em Engenharia Informática", grau="Licenciatura", credits_number=180,
                         credits_numberByYear="1:60|2:60|3:60", duration=6, timetable="Diurno",
                         coordinator=allTeachers.pop())
    newCourseEI.save()

    # -----------------  couso mini_cursos associaçoes  -----------------
    newCourse_MiniCourse = Course_MiniCourse(course=newCourseEI, miniCourse=newCourseF, credits_number=3, year=2,
                                             semestres="2")
    newCourse_MiniCourse.save()

    newCourse_MiniCourse = Course_MiniCourse(course=newCourseEI, miniCourse=newCourseF, credits_number=3, year=1,
                                             semestres="1")
    newCourse_MiniCourse.save()

    newCourse_MiniCourse = Course_MiniCourse(course=newCourseEI, miniCourse=newCourseOEI, credits_number=6, year=3,
                                             semestres="2")
    newCourse_MiniCourse.save()

    newCourse_MiniCourse = Course_MiniCourse(course=newCourseTI, miniCourse=newCourseF, credits_number=9, year=1,
                                             semestres="1")
    newCourse_MiniCourse.save()

    newCourse_MiniCourse = Course_MiniCourse(course=newCourseTI, miniCourse=minorsOBJ[0], credits_number=30, year=3,
                                             semestres="1,2")
    newCourse_MiniCourse.save()

    newCourse_MiniCourse = Course_MiniCourse(course=newCourseTI, miniCourse=minorsOBJ[1], credits_number=30, year=3,
                                             semestres="1,2")
    newCourse_MiniCourse.save()

    newCourse_MiniCourse = Course_MiniCourse(course=newCourseTI, miniCourse=newCourseOTI, credits_number=6, year=3,
                                             semestres="1,2")
    newCourse_MiniCourse.save()
        ...
        for line in rfile.readlines():
            if "#" not in line and "||" in line:
                ...
                for course in lstCourses:
                    ...
                    newCourseSubject = CourseSubject(course=course, subject=newSubject, year=year, semester=semester,
                                                     type=cleanType)
...
def makeSystemUserCourseOBJs():
    ...
    casosPossiveis = [["2016/2017", "1"], ["2016/2017", "2"], ["2016/2017", "3"], ["2017/2018", "1"],
                      ["2017/2018", "2"], ["2018/2019", "1"]]
    for user in allStudents:
        anoInicio, anoAtual = random.choice(casosPossiveis)
        course = random.choice(allLicenciaturas)
        newSystemUserCourse = SystemUserCourse(user=user, course=course, estadoActual="Matriculado",
                                               anoLectivoDeInício=anoInicio, anoActual=int(anoAtual))
                ...
                if "SystemUserCourse" in line:
                    ...
                    if "#" not in line and "," in line:
                        ...
                        if algoritmo == "SystemUserCourse":
                            ...
                            lstSystemUserCourseObj = SystemUserCourse.objects.filter(course=CourseObj,
                                                                                     estadoActual=estado,
                                                                                     anoLectivoDeInício=anoLetivoInicio,
                                                                                     anoActual=ano)
                            ...
                            if tipoDeInscriçaoSubj == "aprovadas":
                                ...
                                for SystemUserCourseObj in lstSystemUserCourseObj:
                                    ...
                                    for semSubjLessons in line.split("|  |"):
                                        ...
                                        for subjLesson in subjLessons.split("!!"):
                                            ...
                                            # ---------- LessonSystemUser objects ----------
                                            anoLetivoAprov = random.choice(
                                                opcoes.split("||"))  # ano letivo em q foi aprovado a cadeira

                                            schoolYearObj = SchoolYear.objects.get(
                                                begin=int(anoLetivoAprov.split("/")[0]))
                                            newSystemUserSubject = SystemUserSubject(user=sysUser, subject=SubjObj,
                                                                                     subjSemestre=int(semestre[0]),
                                            ...
                                            for typeTurma in lstTypeTurma:
                                                ...
                                                lessonObjs = Lesson.objects.filter(subject=SubjObj, type=type,
                                                                                   turma=turma)
                                                diasDaSemana = ""
                                                for lessonObj in lessonObjs:  # ex: T11 terça, T11 quinta
                                                    diasDaSemana = lessonObj.week_day

                                                    allCorrectDates = lessonSystemUser(anoLetivoAprov, semestre,
                                                                                       diasDaSemana)
                                                    ...
                                                    for presData in presDate_str.split("!!"):
                                                        ...
                                                        newLessonSystemUser = LessonSystemUser(systemUser=sysUser,
                                                                                               lesson=lessonObj,
                                                                                               presente=ast.literal_eval(
                                                                                                   presenca),
                                                                                               date=dataFormat)
                                ...
                                # cada turma vai ter 17 alunos exepto teoricas
                                if variante == 0:
                                ...
                                for SystemUserCourseObj in lstSUPartida:
                                    ...
                                    for semSubjLessons in line.split("|  |"):
                                        ...
                                        for subjLesson in subjLessons.split("!!"):
                                            ...
                                            # ---------- LessonSystemUser objects ----------
                                            schoolYearObj = SchoolYear.objects.get(
                                                begin=2018)  # ele esta a increver-se este ano
                                            newSystemUserSubject = SystemUserSubject(user=sysUser, subject=SubjObj,
                                                                                     subjSemestre=int(semestre[0]),
                                                                                     state=state, grade=None,
                                                                                     turmas=turmas_str,
                                                                                     anoLetivo=schoolYearObj)
                                            newSystemUserSubject.save()
        ...
        for line in rfile.readlines():
            if "#" not in line and "||" in line:
                ...
                for lesson in lstLessons:
                    ...
                    newLesson = Lesson(subject=subject, type=type, turma=turma, week_day=weekDay, hour=hour,
    ...
    for semestre, lstLessons in allLessonsBySem.items():
        for lessons in lstLessons:  # lessons- lista de listas, cada lista tem lessons de uma cadeira de um semestre
            ...
            for lesson in lessons:  # lessons de uma cadeira
                ...
                while is_lesson_sobreposta(allTeachers[i], lesson, semestre) or isTooMuchLessonsOfSameSubj(
                        allTeachers[i], lesson.subject):
