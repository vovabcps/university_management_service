...
def getAlunosOfTypesturmasInSubjs(sem, dicSemSubjs, schoolYearObj, infoSemPresenças=[]):
    ...
    for subj in dicSemSubjs:
        ...
        dicSemSubjs[subj] = getAlunosOfClassesInSpecificSubj(sem, subj, dicSemSubjs[subj], schoolYearObj,
                                                             infoSemPresenças)
    return dicSemSubjs
...
def getAlunosOfClassInSpecificSubj(sem, subj, typeTurma, schoolYearObj, infoSemPresenças=[]):
    ...
    suSubjs = list(SystemUserSubject.objects.filter(subject=subj, turmas__contains=typeTurma, anoLetivo=schoolYearObj,
                                                    subjSemestre=sem))
    ...
    for suSubj in suSubjs:
        ...
        if infoSemPresenças:  # se nao for uma lista vazia
            ...
            for date_str in lstDates:
                ...
                lSU = LessonSystemUser.objects.filter(systemUser=suSubj.user, lesson__type=type, lesson__subject=subj,
                                                      date=dataFormat).first()
...
def getAllAlunosQueTemAulasComUmProf(systemUser, schoolYearObj):
    ...
    for subjName, typeTurma in groupby(lessons, lambda a: a["subject__name"]):
        ...
        for dic in typeTurma:
            suSubject = SystemUserSubject.objects.filter(subject__name=subjName,
                                                         turmas__contains=dic["type"] + dic["turma"],
                                                         anoLetivo=schoolYearObj).values("user")
            if len(suSubject) != 0:
